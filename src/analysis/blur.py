...
OUTPUT_JSON = RESULTS_DIR / "blur_results.json"
...
def evaluate(model, radius):
    ...
    for row in tqdm(test_rows, desc=f"blur_{radius}"):
        img_path = RAW_DATA_DIR / row["path"]
        img = load_image(img_path)
        img_pil = Image.fromarray(img.astype(np.uint8))
        
        # Apply Blur
        img_blurred = blur(img_pil, radius)
        img_np = np.asarray(img_blurred, dtype=np.float32)
...
def main():
    ...
    print(f"[DONE] Blur experiments complete. Results saved to {OUTPUT_JSON}")
