...
from config import TRAIN_CSV, PROCESSED_DATA_DIR, RAW_DATA_DIR
from utils.jpeg_utils import jpeg_compress

CSV_FILE = TRAIN_CSV
OUTPUT_X = PROCESSED_DATA_DIR / "X_train.npy"
OUTPUT_Y = PROCESSED_DATA_DIR / "y_train.npy"
OUTPUT_SRC = PROCESSED_DATA_DIR / "sources_train.npy"
...
for row in tqdm(rows, desc="Extracting features"):
    ...
    for c in range(3):  # R, G, B
        ...
        F = fft_magnitude(cd)
        channel_features.extend(extract_peaks(F))

    X.append(channel_features)
    y.append(row["label"])
...
print("\n[SUCCESS]")
print(f"X shape: {X.shape}  (should be [N, 135])")
